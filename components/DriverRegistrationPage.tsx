      ...
      <div className="mx-auto max-w-5xl px-4">
        <header className="mb-8 md:mb-10 flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-300/80 mt-1">Заполните форму. Документы проверяются диспетчером в течение 24 часов.</p>
          </div>
        ...
        <form onSubmit={onSubmit} className="grid gap-6">
          ...
          <Section title="Личные данные">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <Input id="email" type="email" placeholder="you@mail.ru" value={form.email} onChange={(e) => update("email", e.target.value)} />
              </div>
            </div>
          </Section>
          ...
          <Section title="Водительское удостоверение">
            <div className="grid md:grid-cols-4 gap-4">
              <div className="space-y-2 md:col-span-2">
                ...
                <Input id="licenseNumber" placeholder="99 99 999999" value={form.licenseNumber} onChange={(e) => update("licenseNumber", e.target.value)} />
              ...
              <div className="space-y-2">
                ...
                <Input id="licenseDate" type="date" value={form.licenseDate} onChange={(e) => update("licenseDate", e.target.value)} />
                {errors.licenseDate && <p className="text-sm text-rose-400">{errors.licenseDate}</p>}
              </div>
            </div>
            <div className="grid md:grid-cols-3 gap-4">
              <FileInput id="photo" label="Фото водителя" accept="image/*" onChange={(e) => setFiles((s) => ({ ...s, photo: e.target.files?.[0] || null }))} />
              <FileInput id="licenseFront" label="Права — лицевая сторона" accept="image/*" onChange={(e) => setFiles((s) => ({ ...s, licenseFront: e.target.files?.[0] || null }))} />
              <FileInput id="licenseBack" label="Права — оборотная сторона" accept="image/*" onChange={(e) => setFiles((s) => ({ ...s, licenseBack: e.target.files?.[0] || null }))} />
            </div>
          </Section>
          ...
          <Section title="Автомобиль (если работаете на своём, необязательно)">
            <div className="grid md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Input id="carBrand" placeholder="Hyundai Solaris" value={form.carBrand} onChange={(e) => update("carBrand", e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="carColor">Цвет</Label>
                <Input id="carColor" placeholder="Белый" value={form.carColor} onChange={(e) => update("carColor", e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="carPlate">Госномер</Label>
                <Input id="carPlate" placeholder="А123БВ178" value={form.carPlate} onChange={(e) => update("carPlate", e.target.value)} />
              </div>
            </div>
            <FileInput id="carPhoto" label="Фото автомобиля" accept="image/*" onChange={(e) => setFiles((s) => ({ ...s, carPhoto: e.target.files?.[0] || null }))} />
          </Section>
          ...
          <Section title="Согласия">
            <div className="space-y-3 text-sm text-slate-300">
              <label className="flex items-start gap-3">
                <input type="checkbox" checked={form.consentPD} onChange={(e) => update("consentPD", e.target.checked)} className="mt-1 h-4 w-4" />
                <span>
                  Я даю согласие на обработку персональных данных (ФЗ-152) и загрузку документов для верификации.
                  <br />
                  <a className="underline text-cyan-300" href="#" onClick={(e)=>e.preventDefault()}>Политика конфиденциальности</a>
                </span>
              </label>
              {errors.consentPD && <p className="text-sm text-rose-400">{errors.consentPD}</p>}

              <label className="flex items-start gap-3">
                <input type="checkbox" checked={form.consentOffer} onChange={(e) => update("consentOffer", e.target.checked)} className="mt-1 h-4 w-4" />
                <span>
                  Я принимаю условия публичной оферты / договора ГПХ.
                  <br />
                  <a className="underline text-cyan-300" href="#" onClick={(e)=>e.preventDefault()}>Условия оферты</a>
                </span>
              </label>
          ...
          <div className="flex items-center justify-between gap-4">
            <p className="text-slate-300/80 text-sm">Все данные передаются по защищённому каналу и используются только для подключения к сервису.</p>
