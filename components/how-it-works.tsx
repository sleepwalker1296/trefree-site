          ...
          <div className="relative bg-black/20 backdrop-blur-sm border border-[#00D1FF]/20 rounded-3xl p-8 md:p-12">
                  ...
                  <div className="space-y-3">
                    <h3
                      className={`font-semibold text-sm md:text-base leading-tight transition-all duration-500 ${
                        animatedStep === index ? "text-[#00D1FF]" : "text-[#E8EBFF]"
                      }`}
                    >
                      {step.title}
                    </h3>
                    <p
                      className={`text-xs md:text-sm leading-relaxed transition-all duration-500 ${
                        animatedStep === index ? "text-[#E8EBFF]" : "text-[#E8EBFF]/70"
                      }`}
                    >
                      {step.description}
                    </p>
            ...
            <div className="hidden md:block mt-12 relative">
              <div className="relative h-2">
                <div className="absolute inset-0 border-t-2 border-dotted border-[#00D1FF]/40"></div>
                {steps.map((_, index) => (
                  <div
                    key={index}
                    className={`absolute w-2 h-2 rounded-full -top-1 transition-all duration-500 ${
                      animatedStep === index
                        ? "bg-gradient-to-r from-[#00D1FF] to-[#FF4FD8] scale-150"
                        : "bg-[#00D1FF]/60"
                    }`}
                    style={{
                      left: `calc(${(100 / (steps.length - 1)) * index}% + ${100 / steps.length / 2}%)`,
